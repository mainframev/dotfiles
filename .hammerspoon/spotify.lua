...
local function formatSeconds(seconds)
  ...
  if minutes > 0 then
    formatted = (minutes % 60) .. "m " .. formatted
  end
  if hours > 0 then
...
local function spotifyExec(command)
  command = command or ""
  local output = hs.execute("source ~/.dots/sys/env; ~/go/bin/spotify " .. command)
...
function spotify.display()
  ...
  local time = (formatSeconds(current) .. "  (" .. percent .. "%)" .. "\n" .. formatSeconds(total))
  local info = track .. "\n" .. album .. "\n" .. artist .. "\n" .. time
